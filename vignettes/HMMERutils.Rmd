...
enhance its functionality via packages. `r Biocpkg("HMMERutils")` is a `R` 
...
`r Biocpkg("HMMERutils")` by using the following commands in your `R` session:
...
`r Biocpkg("HMMERutils")` is based on many other packages and in particular in 
those that have implemented the infrastructure needed for dealing with protein 
sequences data. That is, packages like `r Biocpkg("Biostrings")` .

If you are asking yourself the question "Where do I start using Bioconductor?" 
you might be interested in 
[this post](http://lcolladotor.github.io/2014/10/16/startbioc/#.VkOKbq6rRuU).
...
resource for getting help: remember to use the `HMMERutils` tag and check 
[the older posts](https://support.bioconductor.org/t/HMMERutils/). Other 
...
We hope that `r Biocpkg("HMMERutils")` will be useful for your research. Please 
...
The `r Biocpkg("HMMERutils")` package 
`r #Citep(bib[["HMMERutils"]])` was made possible thanks to:
